...
import { classifyClaudeTranscript, createClaudeRawParser } from "../../packages/backfill/src/clauderaw.ts";
...
async function tree(root: string): Promise<Tree> {
  ...
  async function walk(directory: string, depth: number): Promise<void> {
    ...
    for (const name of names.sort()) {
      ...
      else if (name.endsWith(".jsonl") && classifyClaudeTranscript(local) !== undefined) {
...
export async function ingestClaudeRaw(root: string, checkpoint: string, client: RpcClient): Promise<void> {
  root = resolve(root);
  ...
  await ingestSnapshot(checkpoint, client, async () => {
    ...
    async function* records(validating = false): AsyncGenerator<SourceRecord> {
      ...
      for (const name of initial.files) {
        const kind = classifyClaudeTranscript(name)!;
        const parse = createClaudeRawParser({ path: name, ...(kind === "main" ? {} : { sidechain: kind }) }, true);
        const hash = createHash("sha256");
        for await (const { bytes, line } of lines(root, name, initial.fingerprints.get(name)!, hash)) {
          let episodes;
          try { episodes = parse(bytes); }
    ...
    const sourceHash = sha(JSON.stringify({ format: "claude-raw-snapshot/1", manifest, fingerprints: [...initial.fingerprints] }));
    return { sourceHash, records: records(), assertUnchanged };
  });
  console.log(JSON.stringify({ event: "source_scope", source: "claude-raw", snapshot: "complete", live_tail: "unsupported", rotation: "unsupported", producer_sealed: "required" }));
}
