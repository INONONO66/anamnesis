...
function isCodexRollout(path: string): boolean {
  return path.endsWith(".jsonl") && !path.split(sep).some(part => part.startsWith("sessions-"));
}
...
export async function ingestCodexRaw(root: string, checkpoint: string, client: RpcClient): Promise<void> {
  root = resolve(root);
  ...
  await ingestSnapshot(checkpoint, client, async () => {
    ...
    async function* records(validating = false): AsyncGenerator<SourceRecord> {
      ...
      for (const name of initial.files) {
        const parse = createCodexRawParser(name.split("/").pop()?.replace(/\.jsonl$/, "") ?? name, true);
    ...
    const sourceHash = sha(JSON.stringify({ format: "codex-raw-snapshot/1", manifest, fingerprints: [...initial.fingerprints] }));
    return { sourceHash, records: records(), assertUnchanged };
  });
  console.log(JSON.stringify({ event: "source_scope", source: "codex-raw", snapshot: "complete", live_tail: "unsupported", rotation: "unsupported", producer_sealed: "required" }));
}
