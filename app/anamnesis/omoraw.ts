...
import { createOmoRawParser } from "../../packages/backfill/src/omoraw.ts";
...
function isOmoTranscript(path: string): boolean {
  return path.endsWith(".jsonl") && !path.split(sep).some(part => part.startsWith("._"));
}
...
export async function ingestOmoRaw(root: string, checkpoint: string, client: RpcClient): Promise<void> {
  root = resolve(root);
  ...
  await ingestSnapshot(checkpoint, client, async () => {
    ...
    async function* records(validating = false): AsyncGenerator<SourceRecord> {
      ...
      for (const name of initial.files) {
        const parse = createOmoRawParser();
    ...
    const sourceHash = sha(JSON.stringify({ format: "omo-raw-snapshot/1", manifest, fingerprints: [...initial.fingerprints] }));
    return { sourceHash, records: records(), assertUnchanged };
  });
  console.log(JSON.stringify({ event: "source_scope", source: "omo-raw", snapshot: "complete", live_tail: "unsupported", rotation: "unsupported", producer_sealed: "required" }));
}
