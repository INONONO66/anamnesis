...
test("G004 independent oracle on real Node, owned Neo4j and UDS", async () => {
  ...
  const parent = resolve(".omo/evidence/g004-gds-qualification");
  await mkdir(parent, { recursive: true });
  const root = await mkdtemp(join(parent, "node-"));
  for (const [source, name] of [["app/anamnesis/main.ts", "daemon.mjs"], ["scripts/qa/g004-gds-qualification.fixture.mjs", "fixture.mjs"]]) {
  ...
  const result = await startProcess("node", command, { deadlineMs: 180000,
    env: { ...process.env, G004_DAEMON: join(root, "daemon.mjs"), G004_OUTPUT: root }, onOutput: text => process.stdout.write(text) }).done;
