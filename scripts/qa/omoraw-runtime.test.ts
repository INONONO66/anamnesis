...
for (const failAfterIngest of [false, true]) test(`OMO raw private Node daemon/ops UDS runtime${failAfterIngest ? " fixture failure cleanup" : ""}`, async () => {
  ...
  const evidence = resolve(".omo/evidence/omoraw-runtime/real");
  ...
  const root = await mkdtemp("/tmp/ana-omo-runtime-"), source = join(root, "export"), cp = root + "-checkpoint.json";
  ...
  const run = async () => {
    try {
      await mkdir(join(source, "home/.omo/sessions"), { recursive: true });
      const file = join(source, "home/.omo/sessions/session.jsonl");
      ...
      const ops = () => startProcess("node", [`${build}-ops.mjs`, "ingest-omo-raw", source, cp], { env, deadlineMs: 120000 });
      ...
      try { if (client) await client.close(); }
        ...
        try { if (daemon) { daemon.stop(); await daemon.done; } }
          ...
          const receipt = { event: "runtime_fixture_cleanup", family: "omo", failAfterIngest, owned, before, remaining: [], client_closed: !!client, daemon_stopped: !!daemon };
