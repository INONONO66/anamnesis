...
fn question_eval(id: &str, qtype: &str, first_hit: Option<usize>) -> QuestionEvaluation {
    ...
    QuestionEvaluation {
        ...
        reranker_metrics: RetrievalMetrics {
            precision_at_k: if first_hit.is_some() { 1.0 } else { 0.0 },
            recall_at_k: if first_hit.is_some() { 1.0 } else { 0.0 },
            mrr,
            ndcg_at_k: if first_hit.is_some() { 1.0 } else { 0.0 },
        },
        delivered_metrics: RetrievalMetrics::default(),
        rendered_recall: 0.0,
        rendered_hit: false,
        rendered_matched_gold_units: Vec::new(),
        candidate_first_hit_rank: first_hit,
        reranker_first_hit_rank: first_hit,
        candidate_k: 20,
        reranker_k: 20,
        delivered_k: 20,
        ...
        consumer_ranking: Vec::new(),
        consumer_document_fingerprint: None,
        consumer_document_count: None,
...
fn sample_report(model: &str) -> RealBenchReport {
    RealBenchReport {
        ...
        warmup: WarmupReport::default(),
        candidate_metrics: Default::default(),
        reranker_metrics: Default::default(),
        delivered_metrics: Default::default(),
        rendered_recall: 0.0,
        rendered_hit: 0.0,
