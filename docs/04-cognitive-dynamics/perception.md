...
When novelty is below the separation threshold, route to the nearest site. A committed access appends a fresh access trace stamped at `now` to the node's bounded trace history, with a per-trace decay rate `d_j = m_type · ( c · e^{m_j} + α )` computed from the activation `m_j` of the existing traces before the new one is added. Because `B_i` ages all prior traces to `now` before adding the new one, this raises `B_i` — and the activation-dependent per-trace decay is what delivers the genuine spacing effect (spaced re-presentation lands at low activation → low `d_j` → durable strength). The testing effect is not cleanly reproduced by this model (see [dissipation.md](dissipation.md)). The route may also update the evidence prior `P_i` from prediction error:
