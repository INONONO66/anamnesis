...
Each factor is a bounded gate. `contradiction_weight_ij` is the bounded constraint strength carried by the `Contradicts` edge — the projection `project_weight(C_ij) = logistic(C_ij) ∈ (0, 1)` of its conductance reservoir, the same projection that yields `edge_weight_ij`. `min(a_i, a_j)` is the bounded query-local activation, and `scope_overlap`, `temporal_overlap` each lie in `[0, 1]` (`0` = no overlap, `1` = full overlap). So `sigma_ij >= 0`, and if any of these gates is `0` the stress is `0`. See [frustration.md](../04-cognitive-dynamics/frustration.md).
