...
# it resolves the binary (PATH, then ~/.cargo/bin), no-ops if absent, and ALWAYS
# exits 0 so a wrong/old/missing binary can never block or erase a prompt. The
...
BIN=$(command -v anamnesis-mcp 2>/dev/null) || BIN=
