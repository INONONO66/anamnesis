...
For each useful claim, FIRST select one source span, THEN write only what that span supports. Only the speaker's supplied actor identifier may come from metadata. Preserve that Episode's language independently: Japanese content stays Japanese, English stays English, Korean stays Korean, regardless of other Episodes in the batch. Do not default to the batch's dominant language.
