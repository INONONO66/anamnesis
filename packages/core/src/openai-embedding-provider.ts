...
import { EmbeddingConfig, EmbeddingError, validateVector, type EmbeddingProfile, type EmbeddingProvider } from "./embedding.ts";
...
class OpenAiEmbeddingError extends EmbeddingError {
  ...
  constructor(readonly code: EmbeddingError["reason"] | "provider_mismatch") {
    super(code === "provider_mismatch" ? "profile_mismatch" : code);
    this.message = code;
...
export class OpenAiEmbeddingProvider implements EmbeddingProvider {
  ...
  async embed(text: string, purpose: "document" | "query"): Promise<OpenAiEmbeddingResult> {
    ...
    if (countBudget(input, "utf8_bytes") > this.profile.max_input_bytes) throw new OpenAiEmbeddingError("input_too_large");
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new OpenAiEmbeddingError("provider_unavailable");
      }
      if (!response.body) throw new OpenAiEmbeddingError("provider_mismatch");
      ...
      try {
        for (;;) {
          ...
          if (bytes > 256 * 1024) {
            await reader.cancel();
            throw new OpenAiEmbeddingError("provider_mismatch");
          }
    ...
    catch { throw new OpenAiEmbeddingError("provider_mismatch"); }
    const envelope = z.object({ data: z.array(z.object({ embedding: z.unknown() })).min(1) }).safeParse(body);
    if (!envelope.success || envelope.data.data[0]!.embedding === undefined) throw new OpenAiEmbeddingError("provider_mismatch");
    const parsed = z.array(z.number()).length(this.profile.dimensions).safeParse(envelope.data.data[0]!.embedding);
    if (!parsed.success) throw new OpenAiEmbeddingError("invalid_vector");
    const norm = Math.hypot(...parsed.data);
    if (!Number.isFinite(norm) || norm === 0) throw new OpenAiEmbeddingError("invalid_vector");
    ...
    try { vector = validateVector(normalized ? parsed.data.map(value => value / norm) : parsed.data, this.profile); }
      ...
      if (error instanceof EmbeddingError) throw new OpenAiEmbeddingError(error.reason);
      throw error;
    }
